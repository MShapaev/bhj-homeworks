const arr1 = Array.from(document.querySelectorAll('.tab')); // привязываем к переменной массив тегов объектов клика
const arr2 = Array.from(document.querySelectorAll('.tab__content')); // привязываем к переменной массив с тегами содержимого
...
scan.addEventListener('click', (event) => { // вызываем обработчик на клик в который передаем event
    ...
    for (let i = 0; i < arr1.length; i++) { // запускаем цикл для удаления классов отображения информации если по элементу не было клика
        ...
        arr1[def].classList.add('tab_active'); // а элементу по которому кликнули присваиваем необходимый класс
        arr2[def].classList.add('tab__content_active'); // тегу с контентом присваиваем класс отображения контента по тому же индексу, так как они совпадают на всей странице
    }
});